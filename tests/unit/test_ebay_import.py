...
async def test_import_all_creates_listing_and_sale(db):
    res = await ebay_import_service.import_all(db, ebay=_FakeEbay())
    assert res["listings"]["created"] == 1
    assert res["orders"]["created"] == 1
    assert res["orders"]["revenue_eur"] == 14.95

    listing = db.query(Listing).filter_by(ebay_item_id="111").one()
    assert listing.listing_status == "active"
    assert float(listing.price_eur) == 14.95
    assert listing.quantity_available == 220

    # Line-Item-Ebene: tx = orderId-lineItemId, lineItem gesetzt, FULFILLED -> tracking
    sale = db.query(Sale).filter_by(ebay_transaction_id="O-1-L-1").one()
    assert sale.status == "tracking"
    assert sale.ebay_line_item_id == "L-1"
    assert sale.listing_id == listing.id      # Verkauf dem Listing zugeordnet
    assert float(sale.price_eur) == 14.95


async def test_import_is_idempotent(db):
    await ebay_import_service.import_all(db, ebay=_FakeEbay())
    res = await ebay_import_service.import_all(db, ebay=_FakeEbay())
    assert res["listings"]["created"] == 0
    assert res["listings"]["updated"] == 1
    assert res["orders"]["created"] == 0      # kein Duplikat beim zweiten Lauf
    assert db.query(Listing).count() == 1
    assert db.query(Sale).count() == 1        # genau EIN Sale, keine Dublette
...
async def test_self_heal_backfills_ebay_sku(db):
    """Self-Heal: die eBay-Publish-SKU wird IMMER am bestehenden Sale nachgetragen (Voraussetzung
    fuer die positions-basierte Varianten-Vorwahl) – auch wenn schon ein (evtl. veralteter) attr da
    ist; ein vorhandener attr wird dabei NICHT ueberschrieben."""
    from decimal import Decimal
    from app.models import Product
    from app.services import ebay_import_service

    p = Product(aliexpress_url="https://de.aliexpress.com/item/r.html", aliexpress_id="rr",
    ...
    db.add(p); db.flush()
    l = Listing(product_id=p.id, ebay_item_id="900", ebay_sku="AE-900", title_seo="Ring",
                description="d", listing_status="active", price_eur=Decimal("14.95"))
    db.add(l); db.commit()

    fake = _FakeEbayVariant()
    await ebay_import_service.import_all(db, ebay=fake)
    sale = db.query(Sale).filter_by(ebay_order_id="OX", ebay_line_item_id="LX").one()

    # (a) Alt-Sale OHNE Aufloesung -> ebay_sku + deterministische Variante (Blau == V2) nachgetragen
    sale.variant_selected = {"Farbe": "Blau"}; db.commit()
    await ebay_import_service.import_all(db, ebay=fake); db.refresh(sale)
    assert sale.variant_selected.get("ebay_sku") == "AE-900-V2"
    assert sale.variant_selected.get("attr") == "14:2"

    # (b) Sale mit KORREKTER (wertverifizierter) Variante ohne ebay_sku -> SKU nachgetragen,
    #     Zuordnung BLEIBT (verifiziert -> nicht ueberschrieben)
    sale.variant_selected = {"Farbe": "Blau", "attr": "14:2"}; db.commit()
    await ebay_import_service.import_all(db, ebay=fake); db.refresh(sale)
    assert sale.variant_selected.get("ebay_sku") == "AE-900-V2"
    assert sale.variant_selected.get("attr") == "14:2"            # verifiziert -> bleibt


async def test_self_heal_autocorrects_wrong_variant(db):
    """Self-Heal AUTO-KORREKTUR (Nutzerwunsch): ein Sale mit versehentlich FALSCHER, nicht
    wertverifizierter Variante wird beim Sync auf die Publish-Position korrigiert – ohne manuellen
    Klick. (Kaeufer kaufte V2/Blau, faelschlich Rot zugeordnet -> zurueck auf Blau.)"""
    from decimal import Decimal
    from app.models import Product
    from app.services import ebay_import_service
    ...
    await ebay_import_service.import_all(db, ebay=fake)
    sale = db.query(Sale).filter_by(ebay_order_id="OX", ebay_line_item_id="LX").one()
    # versehentlich FALSCH: Rot (14:1) – nicht durch den Kaeuferwert (Blau) gedeckt; ebay_sku bleibt
    sale.variant_selected = {"Farbe": "Blau", "ebay_sku": "AE-900-V2",
                             "attr": "14:1", "options": {"Farbe": "Rot"}, "id": 1}
    db.commit()
    await ebay_import_service.import_all(db, ebay=fake); db.refresh(sale)
    assert sale.variant_selected["attr"] == "14:2"   # auf Position V2 (Blau) auto-korrigiert


async def test_self_heal_respects_locked_variant(db):
    """GELD-SCHUTZ (Review-HIGH): eine vom MENSCHEN bestaetigte Zuordnung (set_sale_variant, locked)
    wird von der Auto-Korrektur NIE zurueckgedreht – auch nicht beim 5-Minuten-Scheduler-Sync."""
    from decimal import Decimal
    from app.models import Product
    from app.services import ebay_import_service, order_service
    ...
    await ebay_import_service.import_all(db, ebay=fake)
    sale = db.query(Sale).filter_by(ebay_order_id="OX", ebay_line_item_id="LX").one()
    # Mensch ordnet bewusst 14:1 zu (locked) – die Position waere 14:2.
    order_service.set_sale_variant(db, sale_id=sale.id, attr="14:1")
    db.refresh(sale)
    assert sale.variant_selected["variant_locked"] is True
    await ebay_import_service.import_all(db, ebay=fake); db.refresh(sale)
    assert sale.variant_selected["attr"] == "14:1"   # menschliche Zuordnung NICHT zurueckgedreht


async def test_import_all_does_not_resurrect_refunded_or_duplicate(db):
    """Regression Vorfall 2026-07-06: Voll-Import darf refunded NICHT ueberschreiben
    und keine Order-Level-Dublette anlegen."""
    await ebay_import_service.import_all(db, ebay=_FakeEbay())
    sale = db.query(Sale).filter_by(ebay_transaction_id="O-1-L-1").one()
    sale.status = "refunded"          # Kaeufer wurde erstattet
    db.commit()

    res = await ebay_import_service.import_all(db, ebay=_FakeEbay())  # Order kommt FULFILLED zurueck
    db.refresh(sale)
    assert sale.status == "refunded"        # KEIN Overwrite auf tracking/delivered
    assert res["orders"]["created"] == 0    # KEINE Dublette
